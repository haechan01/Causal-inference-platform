...
treatment_units = ${JSON.stringify(p.treatment_units || [])}
control_vars = ${JSON.stringify(p.controls || [])}

# Create post-treatment indicator (works for numeric or string time)
...
# Create interaction term (DiD = effect of treatment in post period)
df['did'] = df['post'] * df['treated']

# ── DiD regression with cluster-robust SE at unit level ───────────────────────
...
# ── Visualization: outcome means by group over time ───────────────────────────
fig, ax = plt.subplots(figsize=(10, 6))
treatment_means = df[df['treated'] == 1].groupby(time_var)[outcome_var].mean()
control_means = df[df['treated'] == 0].groupby(time_var)[outcome_var].mean()

ax.plot(treatment_means.index.astype(str), treatment_means.values, 'b-o', label='Treatment Group')
ax.plot(control_means.index.astype(str), control_means.values, 'r-o', label='Control Group')
ax.axvline(x=str(treatment_start), color='orange', linestyle='--', label='Treatment Start')
...
plt.show()`;
  ...
  const generateRCode = () => {
    ...
    return `# Difference-in-Differences Analysis
# Generated by Causal Platform
# Install: install.packages(c("tidyverse", "fixest", "ggplot2"))
...
# ── DiD regression with unit fixed effects and cluster-robust SE ───────────────
${p.controls?.length
...
summ <- summary(model, cluster = ~${p.unit})

# Print results
print(summ)
cat("\\nDiD Estimate:    ", coef(model)["did"], "\\n")
cat("Standard Error:  ", summ$se["did"], "\\n")
cat("P-value:         ", summ$pv["did"], "\\n")
confint_did <- confint(summ)["did", ]
...
ggplot(df_summary, aes(x = .data[[time_var]], y = mean_outcome, color = group, group = group)) +
  geom_line(linewidth = 1) +
  geom_point(size = 3) +
  geom_vline(aes(xintercept = treatment_start), linetype = "dashed", color = "orange") +
  labs(
