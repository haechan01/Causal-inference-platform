...
# ── Input validation ─────────────────────────────────────────────────────────
required_cols = [running_var, outcome_var]${isFuzzy && p.treatment_var ? ` + ['${p.treatment_var}']` : ''}
...
if df[running_var].nunique() < 2:
    raise ValueError("Running variable has zero variance — cannot run RD.")
print(f"Dataset: {len(df)} rows, {(df[running_var] >= cutoff).sum()} above cutoff, "
      f"{(df[running_var] < cutoff).sum()} below cutoff")
...
# MSE-optimal (Calonico-Cattaneo-Titiunik 2014, default)
bw_sel = rdbwselect(df[outcome_var], df['running_centered'],
                    kernel='triangular', bwselect='mserd')
bw_opt = bw_sel.bws['h'][0]
print(f"MSE-optimal bandwidth (CCT):          {bw_opt:.4f}")
print(f"Platform-used bandwidth:              ${bwUsed}")

# Imbens-Kalyanaraman (2012) bandwidth — alternative selector
try:
    bw_ik = rdbwselect(df[outcome_var], df['running_centered'],
                       kernel='triangular', bwselect='IK')
    bw_ik_val = bw_ik.bws['h'][0]
    print(f"Imbens-Kalyanaraman bandwidth (IK):   {bw_ik_val:.4f}")
except Exception:
    bw_ik_val = bw_opt   # fall back to MSE-optimal if IK not supported
    print("IK bandwidth not available in this version of rdrobust; using MSE-optimal.")
...
` : ''}
# ── Input validation ─────────────────────────────────────────────────────────
required_cols <- c(running_var, outcome_var${isFuzzy && p.treatment_var ? `, "${p.treatment_var}"` : ''})
...
if (n_distinct(df[[running_var]]) < 2) stop("Running variable has zero variance.")
cat(sprintf("Dataset: %d rows, %d above cutoff, %d below cutoff\\n",
            nrow(df), sum(df[[running_var]] >= cutoff), sum(df[[running_var]] < cutoff)))
...
# ── Step 1: Optimal bandwidth selection ──────────────────────────────────────
# MSE-optimal bandwidth (Calonico-Cattaneo-Titiunik 2014, default)
bw_sel <- rdbwselect(df[[outcome_var]], df$running_centered,
                     kernel = "triangular", bwselect = "mserd")
bw_opt <- bw_sel$bws["h", "left"]
cat(sprintf("MSE-optimal bandwidth (CCT): %.4f  |  Platform used: ${bwUsed}\\n", bw_opt))

# Imbens-Kalyanaraman (2012) bandwidth — alternative selector
bw_ik_val <- tryCatch({
  bw_ik <- rdbwselect(df[[outcome_var]], df$running_centered,
                      kernel = "triangular", bwselect = "IK")
  ik_val <- bw_ik$bws["h", "left"]
  cat(sprintf("Imbens-Kalyanaraman bandwidth (IK): %.4f\\n", ik_val))
  ik_val
}, error = function(e) {
  cat("IK bandwidth not available in this version of rdrobust; using MSE-optimal.\\n")
  bw_opt
})
...
` : ''}
* ── Input validation ────────────────────────────────────────────────────────
foreach v in ${p.running_var} ${p.outcome_var}${isFuzzy && p.treatment_var ? ` ${p.treatment_var}` : ''} {
...
drop if missing(${p.running_var}) | missing(${p.outcome_var})
count
...
display "MSE-optimal bandwidth (CCT): " %9.4f \`bw_opt' "  (platform used: ${bwUsed})"

* Imbens-Kalyanaraman (2012) bandwidth — alternative selector
capture rdbwselect ${p.outcome_var} running_centered, c(0) kernel(triangular) bwselect(ik)
if _rc == 0 {
    local bw_ik = r(h_ik)
    display "Imbens-Kalyanaraman bandwidth (IK): " %9.4f \`bw_ik'
}
else {
    display "IK bandwidth not available in this version of rdrobust"
    local bw_ik = \`bw_opt'
}
