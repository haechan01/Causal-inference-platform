...
# }`;
  ...
  const generateStataCode = () => {
    ...
    const res = results.results || {};
    const polyOrder = res.polynomial_order || 1;
    const bwUsed = res.bandwidth_used ? res.bandwidth_used.toFixed(4) : '.';
    const belowCutoff = p.treatment_side === 'below';
    return `* Regression Discontinuity Design Analysis
* Generated by Causal Platform
* Requires: ssc install rdrobust   (Calonico, Cattaneo & Titiunik, 2014)
...
local treatment_side "${p.treatment_side}"  // "above": treated if >= cutoff
...
* Stored bandwidth: r(h_mserd)

* ── Step 2: Main RD estimate (conventional + bias-corrected robust CI) ────────
rdrobust ${p.outcome_var} running_centered, c(0) kernel(triangular) bwselect(mserd) p(${polyOrder}) h(${bwUsed})
* h(.) uses the optimal bandwidth; replace with a numeric value for fixed BW

ereturn list   // inspect all stored results
...
rdplot ${p.outcome_var} running_centered, c(0) p(${polyOrder}) ///
  ...
  ytitle("${p.outcome_var}") ///
  title("Regression Discontinuity Plot")
graph export "rd_plot.png", replace width(2000)

* ── Step 4: Manipulation test (rddensity) ────────────────────────────────────
* Tests for bunching around the cutoff (strategic sorting)
rddensity running_centered, c(0)
display "If p < 0.05, units may be sorting around the cutoff"

* ── Step 5: Sensitivity to bandwidth choice ───────────────────────────────────
* Re-run rdrobust with half, three-quarter, standard, and larger bandwidths
...
* ── Step 6: Covariate balance check at cutoff (placebo test) ─────────────────
* Replace covariate1, covariate2 with actual pre-treatment covariate names
* foreach cov in covariate1 covariate2 {
*     rdrobust \`cov' running_centered, c(0) p(${polyOrder})
*     display "\`cov': effect=" %9.4f e(tau_cl) "  p=" %6.4f e(pv_rb)
* }`;
  };
