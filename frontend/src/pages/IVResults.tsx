...
const IVResults: React.FC = () => {
  ...
  const generatePythonCode = () => {
    ...
    const hasAdditionalEndog = !!(p.additional_endogenous && p.additional_endogenous.length > 0);
    const hasInteractions = !!(p.interactions && p.interactions.length > 0);
    const instrumentsList = JSON.stringify(p.instruments || []);
    const controlsList    = JSON.stringify(p.controls    || []);
    const addEndogVars    = (p.additional_endogenous || []).map((e: any) => e.variable || e);
    const addEndogInstrs  = (p.additional_endogenous || []).map((e: any) => e.instrument || e);
    const ivEffect  = res.treatment_effect    != null ? Number(res.treatment_effect).toFixed(4)    : 'N/A';
    const ivSE      = res.se                  != null ? Number(res.se).toFixed(4)                  : 'N/A';
    const ivP       = res.p_value             != null ? Number(res.p_value).toFixed(4)             : 'N/A';
    const ivCiLo    = res.ci_lower            != null ? Number(res.ci_lower).toFixed(4)            : 'N/A';
    const ivCiHi    = res.ci_upper            != null ? Number(res.ci_upper).toFixed(4)            : 'N/A';
    const fsF       = res.first_stage?.f_stat != null ? Number(res.first_stage.f_stat).toFixed(2)  : 'N/A';
    const addEndogSection = hasAdditionalEndog ? `
# Additional endogenous variables (multiple endogenous regressors)
additional_endog  = ${JSON.stringify(addEndogVars)}
additional_instrs = ${JSON.stringify(addEndogInstrs)}
all_endog  = [treatment_var] + additional_endog
all_instrs = instruments + additional_instrs

X_endog = df[all_endog]   # all endogenous regressors
Z_instr = df[all_instrs]  # all instruments
` : `
X_endog = df[[treatment_var]]  # endogenous treatment
Z_instr = df[instruments]       # instruments
`;
    const interactionsSection = hasInteractions ? `
# Interaction terms specified in the analysis
interactions = ${JSON.stringify(p.interactions || [])}
for pair in interactions:
    col = f"{pair[0]}_x_{pair[1]}"
    df[col] = df[pair[0]] * df[pair[1]]
    if col not in controls:
        controls.append(col)
` : '';
    return `# Instrumental Variables (2SLS) Analysis
# Generated by CausAl Studio
# Platform result:  2SLS = ${ivEffect},  SE = ${ivSE},  p = ${ivP},  95% CI = [${ivCiLo}, ${ivCiHi}]
# First-Stage F = ${fsF}
...
first_stage = sm.OLS(${hasAdditionalEndog ? 'df[all_endog]' : 'df[[treatment_var]]'}, X_first).fit()
print("=== FIRST STAGE REGRESSION ===")
print(first_stage.summary())
print(f"\\nFirst-Stage F-statistic: {first_stage.fvalue:.4f}  (p = {first_stage.f_pvalue:.4f})")
print("  → F > 10 required;  F > 16.38 recommended (Stock-Yogo threshold)")
strength = "STRONG ✓" if first_stage.fvalue > 10 else "WEAK — interpret with caution ⚠"
print(f"  → Instrument(s): {strength}")
...
X_ols = sm.add_constant(pd.concat(
    [df[[treatment_var]], df[controls]] if controls else [df[[treatment_var]]], axis=1
))
...
print(f"Difference:    {iv_coef - ols_coef:+.4f}  (endogeneity bias)")
...
print("  → p >= 0.05: endogeneity not detected → OLS may also be valid")
...
print("  → p >= 0.05: instruments pass overidentification check ✓")
` : '# Step 5: Sargan-Hansen not applicable (just-identified model)'}
# ── Step 6: Sensitivity Analysis ─────────────────────────────────────────────
${!hasMultipleInstruments ? `# Anderson-Rubin confidence interval (robust to weak instruments)
# The AR CI is valid even if the instrument is weak.
try:
    ar = iv_model.anderson_rubin_ci(significance=0.05)
    print(f"\\nAnderson-Rubin 95% CI (robust to weak instruments): {ar}")
except Exception as e:
    print(f"\\nCould not compute Anderson-Rubin CI: {e}")` : `# Leave-one-out instrument sensitivity
# Drop one instrument at a time; check if 2SLS estimate remains stable.
for drop_instr in instruments:
    remaining = [z for z in instruments if z != drop_instr]
    if not remaining:
        continue
    Z_loo = df[remaining]
    try:
        m_loo = IV2SLS(y, X_exog, X_endog, Z_loo).fit(cov_type='robust')
        loo_coef = m_loo.params[treatment_var]
        print(f"Drop '{drop_instr}': estimate = {loo_coef:.4f}  "
              f"(vs full model {iv_coef:.4f},  diff = {loo_coef - iv_coef:+.4f})")
    except Exception as e:
        print(f"Drop '{drop_instr}': failed — {e}")`}

# ── Step 7: Visualizations ────────────────────────────────────────────────────
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

# Panel A: OLS vs 2SLS coefficient comparison with 95% CI error bars
estimates = {'OLS': ols_coef, '2SLS': iv_coef}
errors    = {'OLS': ols_model.bse[treatment_var] * 1.96, '2SLS': iv_se * 1.96}
ax = axes[0]
...
ax.set_title('OLS vs 2SLS: Effect of ${p.treatment} on ${p.outcome}')
...
# Panel B: First stage — first instrument vs treatment
ax2 = axes[1]
ax2.scatter(df[instruments[0]], df[treatment_var], alpha=0.35, s=20, color='#4F9CF9')
z = np.polyfit(df[instruments[0]].dropna(), df.loc[df[instruments[0]].notna(), treatment_var], 1)
x_line = np.linspace(df[instruments[0]].min(), df[instruments[0]].max(), 100)
ax2.plot(x_line, np.polyval(z, x_line), color='#043873', linewidth=1.5)
ax2.set_xlabel('${p.instruments?.[0] ?? 'Instrument'}')
ax2.set_ylabel('${p.treatment} (treatment)')
ax2.set_title('First Stage: ${p.instruments?.[0] ?? 'Instrument'} vs ${p.treatment}')
...
plt.show()`;
  ...
  const generateRCode = () => {
    ...
    const addEndogVars   = (p.additional_endogenous || []).map((e: any) => `"${e.variable || e}"`).join(', ');
    const addEndogInstrs = (p.additional_endogenous || []).map((e: any) => `"${e.instrument || e}"`).join(', ');
    const ivEffect = res.treatment_effect != null ? Number(res.treatment_effect).toFixed(4) : 'N/A';
    const ivP      = res.p_value          != null ? Number(res.p_value).toFixed(4)          : 'N/A';
    const ivCiLo   = res.ci_lower         != null ? Number(res.ci_lower).toFixed(4)         : 'N/A';
    const ivCiHi   = res.ci_upper         != null ? Number(res.ci_upper).toFixed(4)         : 'N/A';
    const fsF      = res.first_stage?.f_stat != null ? Number(res.first_stage.f_stat).toFixed(2) : 'N/A';
    // Build IV formula parts
    const allEndogR   = hasAdditionalEndog
      ? `c(treatment_var, additional_endog)`
      : `treatment_var`;
    const allInstrsR  = hasAdditionalEndog
      ? `c(instruments, additional_instrs)`
      : `instruments`;
    return `# Instrumental Variables (2SLS) Analysis
# Generated by CausAl Studio
# Platform result:  2SLS = ${ivEffect},  p = ${ivP},  95% CI = [${ivCiLo}, ${ivCiHi}]
# First-Stage F = ${fsF}
# Install: install.packages(c("AER", "estimatr", "tidyverse", "ggplot2"))

library(tidyverse)
library(AER)       # ivreg() + diagnostics (Wu-Hausman, Sargan)
library(estimatr)  # iv_robust(), lm_robust() with HC-robust SEs
library(ggplot2)
...
outcome_var   <- "${p.outcome}"
treatment_var <- "${p.treatment}"
instruments   <- c(${instrumentsR})   # instruments (exclusion restriction required)
controls      <- c(${controlsR})      # exogenous controls
${hasAdditionalEndog ? `additional_endog  <- c(${addEndogVars})
additional_instrs <- c(${addEndogInstrs})
all_endog  <- c(treatment_var, additional_endog)
all_instrs <- c(instruments, additional_instrs)
` : ''}${hasInteractions ? `interactions <- ${JSON.stringify(p.interactions || [])}
for (pair in interactions) {
  col <- paste0(pair[[1]], "_x_", pair[[2]])
  df[[col]] <- df[[pair[[1]]]] * df[[pair[[2]]]]
...
instr_part <- paste(c(${allInstrsR}, if (length(controls) > 0) controls), collapse = " + ")
iv_formula <- as.formula(paste("${p.outcome} ~", exog_part, "|", instr_part))

# ── Step 1: First Stage ───────────────────────────────────────────────────────
fs_formula <- as.formula(paste(
  treatment_var, "~",
  paste(c(${allInstrsR}, if (length(controls) > 0) controls), collapse = " + ")
))
first_stage <- lm(fs_formula, data = df)
fstat <- summary(first_stage)$fstatistic
f_val <- fstat[1]
f_p   <- pf(f_val, fstat[2], fstat[3], lower.tail = FALSE)
cat(sprintf("First-Stage F: %.4f  (p = %.4f)  → %s\\n", f_val, f_p,
            ifelse(f_val > 10, "STRONG ✓", "WEAK ⚠")))
...
cat(sprintf("\\nOLS:  %.4f  |  2SLS: %.4f  |  Diff: %+.4f\\n",
            ols_coef, iv_coef, iv_coef - ols_coef))
...
cat("  → p < 0.05: endogenous → IV necessary\\n")
${hasMultipleInstruments ? `
# ── Step 5: Sargan-Hansen Overidentification Test ────────────────────────────
cat("\\n=== SARGAN-HANSEN OVERIDENTIFICATION TEST ===\\n")
print(diag_tbl["Sargan", ])
cat("  → p >= 0.05: instruments pass overidentification check ✓\\n")
` : '# Step 5: Sargan-Hansen not applicable (just-identified)'}
# ── Step 6: Sensitivity Analysis ─────────────────────────────────────────────
${!hasMultipleInstruments ? `# Anderson-Rubin confidence interval (valid even with weak instruments)
tryCatch({
  ar_test <- ivmodel::KClass(ivmodel::ivmodelFormula(iv_formula, data = df), k = 0)
  cat("Anderson-Rubin CI (robust to weak instruments): run ivmodel::AR.test()\\n")
}, error = function(e) {
  cat("For Anderson-Rubin CI, install the ivmodel package: install.packages('ivmodel')\\n")
})` : `# Leave-one-out instrument sensitivity
cat("\\n=== LEAVE-ONE-OUT INSTRUMENT SENSITIVITY ===\\n")
for (drop_instr in instruments) {
  remaining <- setdiff(instruments, drop_instr)
  if (length(remaining) == 0) next
  loo_instr_part <- paste(c(remaining, if (length(controls) > 0) controls), collapse = " + ")
  loo_formula    <- as.formula(paste("${p.outcome} ~", exog_part, "|", loo_instr_part))
  tryCatch({
    loo_m    <- iv_robust(loo_formula, data = df, se_type = "HC2")
    loo_coef <- coef(loo_m)[treatment_var]
    cat(sprintf("Drop '%s': estimate = %.4f  (diff = %+.4f)\\n",
                drop_instr, loo_coef, loo_coef - iv_coef))
  }, error = function(e) cat(sprintf("Drop '%s': failed\\n", drop_instr)))
}`}
...
ggplot(est_df, aes(x = Method, y = Estimate, color = Method)) +
  geom_point(size = 5) +
  geom_errorbar(aes(ymin = Estimate - 1.96 * SE, ymax = Estimate + 1.96 * SE),
                width = 0.25, linewidth = 1.2) +
  geom_hline(yintercept = 0, linetype = "dashed", alpha = 0.5) +
  geom_text(aes(label = sprintf("%.3f", Estimate)), vjust = -1.5, fontface = "bold", size = 4) +
  scale_color_manual(values = c("OLS" = "#FF6B6B", "2SLS" = "#4F9CF9")) +
  labs(title    = "OLS vs 2SLS: Effect of ${p.treatment} on ${p.outcome}",
       subtitle = "Error bars = 95% confidence intervals",
       y = "Estimated Effect", x = "Estimator") +
  theme_minimal() + theme(legend.position = "none")
ggsave("iv_comparison.png", width = 8, height = 5, dpi = 300)

ggplot(df, aes(x = .data[[instruments[1]]], y = .data[[treatment_var]])) +
  ...
  labs(title = paste0("First Stage: ", instruments[1], " vs ", treatment_var),
       x = instruments[1], y = treatment_var) +
  theme_minimal()
ggsave("iv_first_stage.png", width = 8, height = 5, dpi = 300)`;
  };
  ...
  const generateStataCode = () => {
    ...
    const ivCiLo   = res.ci_lower         != null ? Number(res.ci_lower).toFixed(4)         : 'N/A';
    const ivCiHi   = res.ci_upper         != null ? Number(res.ci_upper).toFixed(4)         : 'N/A';
    const allEndog   = hasAdditionalEndog ? `${p.treatment} ${addEndogVars}` : p.treatment;
    const allInstrs  = hasAdditionalEndog ? `${instrumentsStata} ${addEndogInstrs}` : instrumentsStata;
    return `* Instrumental Variables (2SLS) Analysis
* Generated by CausAl Studio
* Platform result: 2SLS = ${ivEffect}, p = ${ivP}, 95% CI = [${ivCiLo}, ${ivCiHi}]
...
${hasAdditionalEndog ? `local add_endog   "${addEndogVars}"
local add_instrs  "${addEndogInstrs}"
...
display cond(e(F) > 10, "  → STRONG instruments ✓", "  → WEAK instruments ⚠")

* ── Step 2: 2SLS Estimation ─────────────────────────────────────────────────
ivregress 2sls ${p.outcome}${hasControls ? ` ${controlsStata}` : ''} ///
    (${allEndog} = ${allInstrs}), robust

display "Treatment Effect (2SLS): " %9.4f _b[${p.treatment}]
display "Standard Error:          " %9.4f _se[${p.treatment}]
lincom ${p.treatment}   /* estimate + 95% CI */

* ── Step 3: First-stage F from IV context (Stock-Yogo weak instrument test) ──
estat firststage
* Stock-Yogo rule: F > 10 (or > 16.38 for 1 instrument, 5% maximal bias)

* ── Step 4: Wu-Hausman Endogeneity Test ──────────────────────────────────────
estat endogenous
display "  → p < 0.05: endogenous → IV is necessary"
display "  → p >= 0.05: endogeneity not detected"
${hasMultipleInstruments ? `
* ── Step 5: Sargan-Hansen Overidentification Test ───────────────────────────
estat overid
display "  → p >= 0.05: instruments pass overidentification check ✓"
` : `* Step 5: Sargan-Hansen not applicable (just-identified model)`}
* ── Step 6: OLS for comparison ──────────────────────────────────────────────
regress ${p.outcome} ${p.treatment}${hasControls ? ` ${controlsStata}` : ''}, robust
local ols_b = _b[${p.treatment}]
display "OLS Estimate:  " %9.4f \`ols_b' "  (compare with 2SLS = ${ivEffect})"

* ── Step 7: Sensitivity Analysis ────────────────────────────────────────────
${!hasMultipleInstruments ? `* Anderson-Rubin test (robust to weak instruments)
* Requires the weakiv package: ssc install weakiv
weakiv ivregress 2sls ${p.outcome}${hasControls ? ` ${controlsStata}` : ''} ///
    (${p.treatment} = ${instrumentsStata}), robust
* AR 95% CI is valid regardless of instrument strength` : `* Leave-one-out instrument sensitivity
${(p.instruments || []).map((instr: string) => {
  const remaining = (p.instruments || []).filter((i: string) => i !== instr).join(' ');
  return remaining
    ? `ivregress 2sls ${p.outcome}${hasControls ? ` ${controlsStata}` : ''} (${p.treatment} = ${remaining}), robust
display "Drop '${instr}': " %9.4f _b[${p.treatment}] " (vs full 2SLS = ${ivEffect})"`
    : `* Cannot drop '${instr}' — would leave zero instruments`;
}).join('\n')}`}

* ── Step 8: Visualization — OLS vs 2SLS ────────────────────────────────────
                ...
                {showCode && (
                  <div style={styles.codeContent}>
                    <p style={styles.codeDescription}>
                      Reproduce this analysis in your own environment. Includes first stage, 2SLS estimate, OLS comparison, Wu-Hausman endogeneity test, overidentification test (if applicable), sensitivity analysis, and visualizations — with all variable selections, controls, interactions, and additional endogenous variables applied.
                    </p>
