...
plt.show()`;
  ...
  const generateRCode = () => {
    ...
    const hasControls = p.controls && p.controls.length > 0;
    const hasMultipleInstruments = p.instruments && p.instruments.length > 1;
    const instrumentsFormula = (p.instruments || []).join(' + ');
    const controlsFormula = hasControls ? ' + ' + (p.controls || []).join(' + ') : '';
    return `# Instrumental Variables (2SLS) Analysis
# Generated by Causal Platform
                ...
                <div style={styles.infoCard}>
                  ...
                  <div style={styles.fStatBarWrapper}>
                    ...
                    <div style={{
                      ...
                      left: `${Math.min(98, Math.max(1, ((res.first_stage.f_statistic || 0) / 40) * 100))}%`,
                    }}>
                      <div style={styles.fStatBarPin} />
                      <div style={styles.fStatBarPinLabel}>
                        F = {res.first_stage.f_statistic?.toFixed(1)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ ...styles.plainEnglishBox, marginTop: '14px', marginBottom: '0' }}>
                    ...
                    <p style={styles.plainEnglishText}>
                      The F-statistic measures how strongly your instrument{parameters.instruments?.length > 1 ? 's' : ''} predict{parameters.instruments?.length === 1 ? 's' : ''} the treatment variable{' '}
                      <strong>{parameters.treatment}</strong>. Think of it as a signal-to-noise ratio —
                      higher is better. The conventional threshold of F &gt; 10 (or ideally &gt; 16)
                      ensures the instrument provides enough information to produce reliable estimates.
                      {isWeak ? ' With weak instruments, the 2SLS estimate can be biased toward the regular (OLS) estimate, which defeats the purpose of IV.' : ''}
                    </p>
                  </div>
                    ...
                    <div style={styles.codeBlock}>
                      <div style={styles.codeBlockHeader}>
                        <span style={styles.codeBlockTitle}>
                          {codeLanguage === 'python' ? 'Python (linearmodels + statsmodels)' : 'R (AER + estimatr)'}
                        </span>
                        <button
                          onClick={() => {
