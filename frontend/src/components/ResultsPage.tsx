...
import BottomProgressBar from './layout/BottomProgressBar';
import { useProgressStep } from '../hooks/useProgressStep';
import { aiService, ResultsInterpretation } from '../services/aiService';
import { useAuth } from '../contexts/AuthContext';
import { projectStateService } from '../services/projectStateService';
import InteractiveDiDChart from './charts/InteractiveDiDChart';
...
const ResultsPage: React.FC = () => {
  ...
  const generateAISummary = () => {
    ...
    const pValueText = pValue !== null && pValue !== undefined ? ` (p = ${formatPValue(pValue)})` : '';
...
ggsave("did_chart.png", width = 10, height = 6, dpi = 300)`;
  ...
  const generateStataCode = () => {
    if (!results?.parameters) return '';
    const p = results.parameters;
    const treatmentUnitsStata = (p.treatment_units || []).map((u: string) => `"${u}"`).join(' ');
...
local treat_start "${p.treatment_start}"

* ── Create post-treatment indicator ───────────────────────────────────────────
gen post = (${p.time} >= "${p.treatment_start}")

* ── Create treatment group indicator ──────────────────────────────────────────
gen treated = inlist(${p.unit}${treatmentUnitsStata ? `, ${treatmentUnitsStata}` : ''})
...
* ── Run DiD regression with clustered standard errors ─────────────────────────
${controlsStata
  ? `reghdfe ${p.outcome} treated post did ${controlsStata}, absorb(${p.unit}) vce(cluster ${p.unit})`
  : `regress ${p.outcome} treated post did, vce(cluster ${p.unit})`}

* ── Display key results ───────────────────────────────────────────────────────
display "DiD Estimate:    " %9.4f _b[did]
display "Standard Error:  " %9.4f _se[did]
display "t-statistic:     " %9.4f _b[did]/_se[did]
lincom did   // prints coefficient + 95% CI

* ── Parallel trends: event study plot ─────────────────────────────────────────
* Requires ssc install event_study  (or use xtdidregress / bacondecomp)
* gen rel_time = ${p.time} - ${p.treatment_start}   // relative time to treatment
* event_study ${p.outcome}, timevar(rel_time) treatvar(treated) unitvar(${p.unit}) timevar(${p.time})

* ── Visualization ─────────────────────────────────────────────────────────────
preserve
  collapse (mean) ${p.outcome}, by(${p.time} treated)
  twoway ///
    (connected ${p.outcome} ${p.time} if treated == 1, lcolor(blue) mcolor(blue) lpattern(solid) legend(label(1 "Treatment"))) ///
    (connected ${p.outcome} ${p.time} if treated == 0, lcolor(red)  mcolor(red)  lpattern(solid) legend(label(2 "Control")))  ///
    , xline(${p.treatment_start}, lpattern(dash) lcolor(orange)) ///
      title("Difference-in-Differences Analysis") ///
                        ...
                        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '14px' }}>
                          <thead>
                            <tr style={{ backgroundColor: '#f8f9fa', borderBottom: '2px solid #dee2e6' }}>
                              ...
                              <th style={{ padding: '12 px', textAlign: 'right', fontWeight: '600', color: '#212529' }}>
                                95% CI Lower
                  ...
                  <div style={styles.codeBlock}>
                    <div style={styles.codeBlockHeader}>
                      <span style={styles.codeBlockTitle}>
                        {codeLanguage === 'python' ? 'Python (statsmodels)' : codeLanguage === 'r' ? 'R (fixest)' : 'Stata (regress / reghdfe)'}
                      </span>
                      <button
                        onClick={() => {
